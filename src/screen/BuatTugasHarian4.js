...
class BuatTugasHarian4 extends React.Component {
  state = {
    data: [],
    refreshing: false,
    status: true,
  ...
  getData = () => {
    ...
    axios
      .get(`https://api.pondokprogrammer.com/api/daily_task`, {
        headers: {
  ...
  buatStandarKompetensi = () => {
    ...
    if (this.state.valueStandarKompetensi != '') {
      ...
      fetch('https://api.pondokprogrammer.com/api/daily_task', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          task: this.state.valueStandarKompetensi,
        }),
      })
        .then(res => res.json())
        .then(json => {
  ...
  ubahStandarKompetensi = () => {
    ...
    if (this.state.valueStandarKompetensiUpdate != '') {
      ...
      fetch(
        `https://api.pondokprogrammer.com/api/daily_task/${
        ...
        }`,
        {
          ...
          body: JSON.stringify({
            topik_id: this.state.topik_id,
            task: this.state.valueStandarKompetensiUpdate,
          }),
        },
      )
        .then(res => res.json())
  ...
  deleteData = paramID => {
    ...
    fetch(`https://api.pondokprogrammer.com/api/daily_task/${paramID}`, {
      method: 'DELETE',
      headers: {
        'Content-Type': 'application/json',
        Authorization: `Bearer${token}`,
        ...
        if (value.topik_id == this.state.topik_id) {
          return (
            <TouchableOpacity
              ...
              delayPressIn={10}
              key={key}
              onPress={() => this.fieldUpdate(value.task, value.id)}>
              <View style={styles.ListBox} key={key}>
                <Text style={styles.boxTitle}>{value.task}</Text>
              </View>
            </TouchableOpacity>
          );
        }
        ...
        <Modal
          ...
          <View style={styles.centeredView}>
            <View style={styles.modalContainer}>
              <TextInput
                multiline={true}
                placeholder="Tuliskan Tugas Harian"
...
export default connect(mapStateToProps)(BuatTugasHarian4);
